  ...
  try {
    ...
    return NextResponse.json({
      ...
      mpAccessToken: config.mpAccessToken
        ? maskSecret(config.mpAccessToken)
        : "",
      mpPublicKey: config.mpPublicKey || "",
      mpWebhookSecret: config.mpWebhookSecret
        ? maskSecret(config.mpWebhookSecret)
        : "",
    })
  } catch {
    return NextResponse.json(
      { error: "Erro ao buscar configuração de pagamento" },
      { status: 500 },
    )
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const existing = await db.paymentConfig.findUnique({
      where: { barbershopId },
    })
    ...
    else if (existing?.mpAccessToken)
      data.mpAccessToken = existing.mpAccessToken
    ...
    if (mpWebhookSecret && !mpWebhookSecret.includes("•"))
      data.mpWebhookSecret = mpWebhookSecret
    else if (existing?.mpWebhookSecret)
      data.mpWebhookSecret = existing.mpWebhookSecret

    const config = await db.paymentConfig.upsert({
      where: { barbershopId },
      create: { ...data, id: randomUUID() } as Parameters<
        typeof db.paymentConfig.create
      >[0]["data"],
    ...
    console.error(e)
    return NextResponse.json(
      { error: "Erro ao salvar configuração de pagamento" },
      { status: 500 },
    )
  }
}
...
function maskSecret(value: string): string {
  ...
  return (
    value.substring(0, 4) +
    "•".repeat(Math.min(20, value.length - 8)) +
