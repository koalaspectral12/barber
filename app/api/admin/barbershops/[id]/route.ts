...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!b) return NextResponse.json({ error: "Barbearia não encontrada" }, { status: 404 })
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const b = await db.barbershop.update({
      where: { id: params.id },
      data: { name, address, phones: serializePhonesField(phones || []), description, imageUrl },
    })
    return NextResponse.json({ ...b, phones: parsePhonesField(b.phones) })
  } catch {
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!ctx || ctx.role !== "SUPERADMIN") {
      return NextResponse.json({ error: "Apenas o Superadmin pode excluir barbearias" }, { status: 403 })
    }

    await db.barbershopService.deleteMany({ where: { barbershopId: params.id } })
